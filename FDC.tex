...
Since right $\Lambda$-modules are the same as left $\Lambda^{\op}$-modules we use these interchangeably. We use the symbol $D$ to denote the duality functor $D: \mod \Lambda \to \mod\Lambda^{\operatorname{op}}$ where $DM = \Hom_k(M, k)$. Typically $D\Lambda$ will refer to the left module $D\Lambda_\Lambda$.
...
When working over a category $\mathcal C$ we will denote the set of morphisms either as $\Hom_{\mathcal C}(M, N)$ or as $\mathcal C(M,N)$. When the ambient category is clear we may also write simply $(M, N)$.

Functors are always assumed to be additive.

%$\Hom_{\mathcal C}(M, N)$ can be written as $\mathcal C(M,N)$ or sometimes simply $(M,N)$

For an exact category $\mathcal A$ we write $\D(\mathcal A)$ to refer to the derived category, $\D^b(\mathcal A)$ to refer to the bounded derived category, $K^b(\mathcal A)$ to refer to the bounded homotopy category, $K^{+,b}(\mathcal A)$ (respectively $K^{-,b}(\mathcal A)$) to refer to the homotopy category of complexes bounded below (respectively above) and bounded in homology. We also write $\D^b(\Lambda)$ instead of $\D^b(\mod\Lambda)$ and $\D(\Lambda)$ instead of $\D(\Mod\Lambda)$. In all of these triangulated categories $X[i]$ will denote the complex $X$ shifted $i$ degrees down. That is, $(X[i])^n = X^{n+i}$. We denote the hard truncation of $X$ by $X^{\geq n}$. The hard truncation is the complex defined by $(X^{\geq n})^m$ equals $X^m$ when $m \geq n$ and 0 otherwise. We define the hard truncation $X^{\leq n}$ similarly.
...
\begin{center}
	...
	\end{tikzcd}
\end{center} 
for its minimal projective resolution. We will let the $n$th syzygies of $M$ be the kernel of $d_M^{n-1}$, denoted by $\Omega^n M$. We also define $\Omega^0 M$ to be $M$.
...
\begin{theorem}\label{thm:Proj_Inj_equivalence}
	...
	\begin{proof}
		...
		Lastly we check well definedness. Since $\Lambda$ is noetherian the direct sum of injectives is injective so 
		$$D\Lambda\otimes (\operatorname{Add}\Lambda) = \operatorname{Add} D\Lambda \subseteq \operatorname{Inj}\Lambda,$$
		and thus the map $D\Lambda \otimes -$ is well defined. 
		
		Similarly since $\Lambda$ is artinian the product of projectives is projective, and so
		$$\Hom(D\Lambda, \operatorname{Prod}D\Lambda) = \operatorname{Prod} \Lambda \subseteq \operatorname{Proj}\Lambda.$$
