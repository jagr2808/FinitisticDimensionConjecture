...
\begin{theorem}\cite[Corollary~8,9]{IgTo05}
	The finitistic dimension conjecture holds for algebras with rep-dimension 3 or less.
	\begin{proof}
		Let $\Lambda$ have rep-dimension 3 or less, that is $\Lambda$ is morita-equivalent to $\End_\Gamma(I_0(\Gamma))$ for an algebra $\Gamma$ with global dimension 3 or less. We will write $I$ for $I_0(\Gamma)$. Let $X$ be any $\Lambda$-module with finite projective dimension. Then it has a projective presentation $(I_1, I) \to (I_0, I) \to X \to 0$ with $I_i \in \add I$. Completing the map $I_0\to I_1$ to an injective resolution in $\Gamma$ we get something on the form $I_0 \to I_1 \to I_2 \to I_3 \to 0$ with $I_2, I_3$ injective (not necessarily in $\add I$). Applying the exact functor $(-,I)$ we get
		\begin{center}
		\begin{tikzcd}
			0 \ar[r] & (I_3, I) \ar[r] & (I_2, I) \ar[r] & (I_1, I) \ar[r] & (I_0, I) \ar[r] & X \ar[r] & 0.
 		\end{tikzcd}
 		\end{center}
 		Thus we have a short exact sequence of $\Lambda$-modules
 		\begin{center}
 		\begin{tikzcd}
 		0 \ar[r] & (I_3, I) \ar[r] & (I_2, I) \ar[r] & \Omega^2 X \ar[r] & 0.
 		\end{tikzcd}
 		\end{center}
 		Then by \cref{thm:projdim_bounded_by_psi} the projective dimension of $\Omega^2 X$ is bounded by $\psi((I_3, I)\oplus (I_2, I))+1$. Which means
 		$$\pd X \leq \psi((I_3, I)\oplus (I_2, I))+3 \leq \psi((D\Gamma, I))+3$$
