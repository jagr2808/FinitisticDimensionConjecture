...
\begin{prop}
	...
	\begin{proof}
		...
		\end{tikzcd}
		\end{center}
		be an injective copresentation of $\Gamma$. Let $P$ be the sum of the projective covers of all nonisomorphic simple modules in the socle of $I_0$. Then $P$ is Wedderburn projective.\todo{prove this}
...
\begin{prop}\label{prop:hom_preserves_injectives}
	...
	\begin{proof}
		...
		\begin{center}
		\begin{tikzcd}
			M \ar[d] \ar[dd, bend right=60, swap, "f"] \ar[dr, "\psi(f)"] \\
			M/K \ar[r] \ar[d] & I\\
			M \ar[ur, dashed, swap, "\hat{f}"]
		\end{tikzcd}
		\end{center}
		Since $M/K$ is injective we have a lift $\hat{f}$. Then we get a lifting of $\psi$ by mapping $1\in\Gamma$ to $\hat{f}$.
		
		Now continue by induction \todo{this is wrong , use the same argument as in the wedderburn case.} assume $J' = J + (f)$ for an ideal $J$ where $(M, I)$ has the lifting property. Then a map $J' \to (M,I)$ is just the sum of maps from $J \to (M, I)$ and $(f) \to (M,I)$, both of which has a lift. Then we get a lift simply by taking the sum of the lifts.
...
\begin{prop}
	...
	\begin{proof}
		...
		\begin{center}
		\begin{tikzcd}
			\bigoplus\limits_i P \ar[r, "\sum f_i"] \ar[rd, swap, "\sum \psi(f_i)"] & I \ar[d, dashed, "?"]\\
			& I'
		\end{tikzcd}
		\end{center}
