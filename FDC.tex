...
\begin{example}\cite[Proposition~2.3]{IST90}\label{exam:not_contravariantly_finite}
	...
	\end{tikzcd}
	\end{center}
	with relations $\alpha \gamma$, $\beta\gamma$, and $\gamma\alpha$. Then $\findim(\Lambda) = 1$, but the subcategory of modules with finite projective dimension is not contravariantly finite.
...
\begin{itemize}
	\item Given a recollement of the bounded derived category you get one for $\Lambda^{op}$. \todo{Look at examples of reccolement to see how it translates.}
...
\begin{itemize}
	...
	\item NC: Certainly $\Lambda$ is self injective if and only $\Lambda^{op}$ is. \todo{Can the dominant dimension of the opposite algebra be different? Arbitrary different?}
