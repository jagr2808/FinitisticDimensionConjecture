...
The generalized Nakayama conjecture is a slight weakening of the Nunke condition. It asserts that none of the simple modules are Nunke modules.
...
\text{VC}\ar[r, Rightarrow] & \text{NuC}\ar[r, Rightarrow] & \text{GNC}\ar[r, Leftrightarrow, "?"] & \text{ARC}\ar[r, Rightarrow] & \text{NC}
\end{tikzcd}
...
\begin{theorem} \cite{Yam96}
	The Auslander--Reiten conjecture implies the generalized Nakayama conjecture.
	\begin{proof}
		Assume that ARC holds, and let $\Gamma$ be a finite dimensional algebra. We wish to show that $\Gamma$ satisfies GNC. Let 
		\begin{center}
		\begin{tikzcd}
		0 \ar[r] & \Gamma \ar[r] & I_0 \ar[r] & I_1 \ar[r] & \cdots
		\end{tikzcd}
		\end{center}
		be the minimal injective resolution of $\Gamma$. Let $I$ be the sum of all non-isomorphic indecomposable modules that appear in the injective resolution, and let $P$ be the projective cover of the socle of $I$. Then by \cref{thm:wedderburn_criterion} we have that $P$ is Wedderburn projective.
		
		Let $\Lambda = \End(P)^{\operatorname{op}}$ and let $M = \Hom(P, \Gamma)$. It's clear that $M$ is a generator. We wish to use ARC to prove that $M$ is projective.
		
		Since $P$ contains the projective cover of all the simples that appear in the socles of $I_i$ we have that
		\begin{center}
			\begin{tikzcd}
			0 \ar[r] & M \ar[r] & (P,I_0) \ar[r] & (P,I_1) \ar[r] & \cdots
			\end{tikzcd}
		\end{center}
		is an injective resolution of $M$. Further we know that $(P, -)$ is faithful on $I_i$.
		\todo{Figure this out}
	\end{proof}
\end{theorem}
...
\begin{prop}\label{prop:conj_on_individual_algebras}
	...
	\begin{enumerate}[i)]
		...
		\item The implications FDC $\Rightarrow$ VC $\Rightarrow$ NuC $\Rightarrow$ GNC, all hold on the level of individual algebras.
...
\begin{prop}\label{prop:hom_generator_preserves_injectives}
	Let $M$ be a module and $I$ an injective module. If the projective cover of the socle of $I$ is in $\add M$, then $(M,I)$ is an injective $\Gamma:=\End(M)^{\operatorname{op}}$-module.
	\begin{proof}
