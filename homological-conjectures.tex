...
\begin{theorem} \cite[Proposition~4.4]{MR04}
	...
	\begin{proof}
		...
		\begin{center}
			...
			\Ext^{n+1}(T_{i}, T) \ar[r] & \Ext^{n+1}(K_{i}, T) \ar[r] & \Ext^n(K_{i+1}, T) \ar[r] & \Ext^n(T_{i}, T)
			\end{tikzcd}
		\end{center}
		Since $T_{i}$ is in $\add T$ we have that $\Ext^n(T_{i})=0$ for all $n>0$. Then by exactness we have that $\Ext^n(K_{i+1}, T) \cong \Ext^{n+1}(K_i, T) = 0$ for all $n>0$. So by induction $\Ext^{>0}(K_i, T)=0$ for all $i\geq 0$.
		...
		\begin{center}
			...
			\end{tikzcd}
		\end{center}
		Since we showed above that $\Ext^{>0}(K_m, T) = 0$ and $T_{m-i-1}$ is in $\add T$ we get that $\Ext^{>0}(K_m, T_{m-i-1}) = 0$. Thus $\Ext^{i}(K_m, K_{m-i}) \cong \Ext^{i+1}(K_m, T_{m-i-1})$, and by induction we have that 
		...
		\begin{center}
			...
			\end{tikzcd}
		\end{center}
		Therefor $\pd K_i \leq \sup\{\pd T_{i-1}, \pd K_{i-1} - 1\} < \infty$.
...
\begin{theorem} \cite[1.2]{Hap93} \label{thm:VC_implies_Nuc}
	...
	\begin{proof}
		...
		Let $I^\bullet \in K^b(\inj\Lambda)$ be a complex of width $n$. without loss of generality we may assume $I^\bullet$ is concentrated in degrees $0 \leq i \leq n-1$. Then $$I^{>0} \to I \to I^{0} \to I^{>0}[1]$$ is a triangle with $I^{>0}$ of width $n-1$ and $I^0$ of width 1. Taking the long exact sequence in $\D^b(-,X)$ it follows that $\D^b(I, X)=0$. Since $X \neq 0$ we have that $\Lambda$ does not satisfy VC.
	\end{proof}
\end{theorem}
...
\begin{enumerate}[i)]
	...
	\item The implications FDC $\Rightarrow$ VC $\Rightarrow$ NuC $\Rightarrow$ SNC, all hold on the level of individual algebras.
	\item If $\Gamma$ satisfies SNC whenever $\Gamma$ is the endomorphism ring of a generator in $\mod\Lambda$, then $\Lambda$ satisfies ARC.
	...
	\item If $\Lambda$ satisfies SNC, then $\Lambda^{\op}$ satisfies NC.
\end{enumerate}
