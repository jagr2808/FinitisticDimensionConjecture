...
\begin{prop}\label{prop:conj_on_individual_algebras}
	...
	\begin{enumerate}[i)]
		...
		\item If $\Gamma$ satisfies GNC whenever $\Gamma = \End_\Lambda(M)$ for a generator $M$ in $\mod\Lambda$, then $\Lambda$ satisfies ARC.
		\item If $\End(I)^{\op}$ satisfies ARC, where $\add I$ contains every injective in the minimal resolution of $\Lambda$, then $\Lambda$ satisfies GNC.
		...
		\item If $\Lambda$ satisfies GNC, then $\Lambda$ satisfies NC.
	\end{enumerate}
\end{prop}
