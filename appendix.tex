...
\begin{theorem}\label{thm:local_artin_ring_Findim_0}
	...
	\begin{proof}
		...
		\begin{center}
			\begin{tikzcd}
			0 \ar[r] & R^{(I')} \ar[r] & R^{(I)} \ar[r] & M \ar[r] & 0
			\end{tikzcd}
		\end{center}
...
\begin{theorem}[Fitting's Lemma]\label{thm:Fittings_lemma}
	...
	\begin{proof}
		...
		Since $X$ is noetherian this sequence stabilizes, i.e. there is an integer $\eta_X$ such that $\ker L^n \cap X = \ker L^{n+1} \cap X$ for all $n \geq \eta_X$. We know that $L^n(X) \cong X / (\ker L^n \cap X)$, and that through this isomorphism the map $L \colon L^n(X) \to M$ is induced by the map $L^{n+1} \colon X / (\ker L^n \cap X) \to L^{n+1}(X) \subseteq M$. Since for $n \geq \eta_X$ we have that $\ker L^n \cap X = \ker L^{n+1}\cap X$ this map is injective, and so the theorem holds.
	\end{proof}
\end{theorem}
