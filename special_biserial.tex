...
\begin{theorem}
	...
	\begin{proof}
		...
		\begin{center}
		\begin{tikzcd}
			0 \ar[r] & K \ar[r]\ar[d]\arrow[dr, phantom, "\usebox\pullback" , very near start, color=black] & P \ar[r]\ar[d] & \cok\varepsilon_M \ar[r] \ar[d] & 0\\
		...
		\begin{center}
			...
			\end{tikzcd}
		\end{center} 
		The only thing we need to show is that the last map is surjective. We do this by verifying the three cases for an indecomposable summand of $V$. Firstly let $W$ be a $\Gamma$-module. Then $\Hom_\Lambda(W, V_2)$ breaks up as a direct sum into $\Hom_\Lambda(W, M') \oplus \Hom_\Lambda(W, P)$ \todo{blabla}
		...
		\begin{center}
			...
			\end{tikzcd}
		\end{center}
		If we let $M$ denote the kernel of $f$ and we choose $V_1$ and $V_2$ as above, then we get a projective resolution of $N$ by
		\begin{center}
			\begin{tikzcd}[column sep=20pt]
			0\ar[r] & (V,V_1) \ar[r] & (V,V_2) \ar[r] & (V,V_1) \ar[r] & (V,V_0) \ar[r] & N \ar[r] & 0.
			\end{tikzcd}
		\end{center}
