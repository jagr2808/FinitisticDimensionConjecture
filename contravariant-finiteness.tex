...
\begin{theorem} \cite[3.8]{AR91} \label{thm:contravariantly_finite_resolving_is_Xi_filtered}
	...
	\begin{proof}
		...
		\begin{center}
			...
			\end{tikzcd}
		\end{center}
		 we get that $X' \to C'$ induces an isomorphism $\Ext^1(X'', X') \to \Ext^1(X'', C)$. Thus the sequence $0 \to C' \to E \to X'' \to 0$  must come from a sequence $0 \to X' \to X \to X'' \to 0$. This gives us a diagram
		\begin{center}
			\begin{tikzcd}
			& 0 \ar[d] &&0 \ar[d] &\\
			 & Y' \ar[d] && Y'' \ar[d] & \\
...
\begin{example}\cite[Proposition~2.3]{IST90}\label{exam:not_contravariantly_finite}
	...
	We see that the preprojective and preinjective modules both have odd dimension, so they have infinite projective dimension as $\Lambda$-modules. We can easily verify that the $\Lambda/(\gamma)$-modules 
	\begin{tikzcd}
